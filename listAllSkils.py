...
def searchskils(query):
    ...
    content = json.loads(response.text)
    content =json.dumps(content, indent=4)
    f = open("Searchskilsbyquery.json", "w+")
    f.write(content)


def searchskils_type(typeId):
    url = "https://skills.emsicloud.com/versions/latest/skills"
    authtok='Bearer '+authentication_token()
    ...
    content = json.loads(response.text)
    content =json.dumps(content, indent=4)
    f = open("searchskilsbytype.json", "w+")
    f.write(content)

def getskils_type():
    url = "https://skills.emsicloud.com/versions/latest/types"
    authtok='Bearer '+authentication_token()
    headers = {'authorization': authtok}
    response = requests.request("GET", url, headers=headers)
    content = json.loads(response.text)
    content =json.dumps(content, indent=4)
    f = open("allskilsbyType.json", "w+")
    f.write(content)

alllistskils()
searchskils(config.searchskil_query)
searchskils_type(config.skilbytype)
getskils_type()
